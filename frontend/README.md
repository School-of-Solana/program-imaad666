...
```shell
npx create-solana-dapp@latest -t gh:solana-foundation/templates/gill/findare
```
...
You can use any normal anchor commands. Either move to the `anchor_project` directory and run the `anchor` command or prefix the
command with `npm run`, eg: `npm run anchor`.

#### Sync the program id:

Running this command will create a new keypair in the `anchor_project/target/deploy` directory and save the address to the
Anchor config file and update the `declare_id!` macro in the `./src/lib.rs` file of the program. This will also update
the constant in `anchor_project/src/basic-exports.ts` file.
